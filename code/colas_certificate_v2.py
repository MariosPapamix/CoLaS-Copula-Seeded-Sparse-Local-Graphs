"""
colas_certificate_v2.py
=======================
Exact outward-rounded interval certificate for the ENLARGED
identification region B = [1,5] x [5,15] x [0, 3/20] in (lambda, eta, psi),
extending the certificate of SI S3 to the full region and adding GLOBAL INJECTIVITY.

Method
------
* Fixed-point dyadic interval arithmetic (PREC=200 bits, outward rounded).
* Exact rational motif coefficients (truncation N=40 per edge, remainder
  bounds from SI S3 at L=5; remainders carried as scalar error budgets).
* Per lambda-cell: rescale lambda = lam0*(1+dmu) exactly, Taylor-shift at 1
  by pure additions (no multiplicative width amplification).
* ALL composite quantities (covariance-type combinations, derivative
  numerators, the injectivity discriminant, the Jacobian determinant and
  compound-matrix numerators) are formed at the COEFFICIENT level, where
  cancellation is exact; each gate is then a single polynomial evaluation
  per subbox (plus small attached error budgets), so interval dependency
  widening is eliminated.
* Adaptive bisection until every gate holds on every leaf.

Certified conclusions
---------------------
 [E-range]  E-enclosure on B2=[1,5]x[0,3/20]; hence ell-image of B inside
            [10*E_lo, 30*E_hi].
 [G1]  on B2 x ell-range:  S>0, den'>0, N_C>0 (C_lambda>0), N_Cp>0
       (C_psi>0), DeltaN>0 (compensated profile slope >0).
       => M3 is INJECTIVE on the closed box B (psi=0 included), by the
          monotone-triangular argument in the revised SI S3.
 [G2]  on B: det DM3 < 0 with certified |det| lower bound, and
       sigma_min(DM3) >= |det| / ||Lambda^2 DM3||_F  >= 1/100.

Reported constants additionally include certified global bounds for
C_lambda, C_psi, the compensated slope, det DM3, sup||DM3||_F^2.

Run:  python3 colas_certificate_v2.py [--selftest] [--quick]
"""
...
class Cell:
    ...
    def build_g1(self):
        ...
        N1 = [tr(TE2l.scale(c34)*SE2 - TE2.scale(c34)*SE2l),
              tr(Al*SE2 + TE2l.scale(c34)*B - A*SE2l - TE2.scale(c34)*Bl),
              tr(Al*B - A*Bl)]
        N2 = [tr(TE2p.scale(c34)*SE2 - TE2.scale(c34)*SE2p),
              tr(Ap*SE2 + TE2p.scale(c34)*B - A*SE2p - TE2.scale(c34)*Bp),
              tr(Ap*B - A*Bp)]
        # DeltaN = N2*NC - N1*NCp  (quadratic in ell); sign of Delta
        DN = [tr(N2[i]*NC - N1[i]*NCp) for i in range(3)]
        den_c = [SE2, B]               # den' = ell*B + SE2
        num_c = [TE2.scale(c34), A]
    ...
    def build_g2(self):
        ...
        num = [TE.scale(c34), A.scale(2)]      # [eta^0, eta^1]
        den = [SE, B.scale(2)]
        num_l = [TEl.scale(c34), Al.scale(2)]
        num_p = [TEp.scale(c34), Ap.scale(2)]
        ...
        DNdet = [x.scale(c34) for x in DNdet]
        ...
        R2n = [[NC.scale(c34)], [PE({}, Fraction(0), Fraction(0))], [NCp.scale(c34)]]
        R3n = [Nr_l, Nr_e, Nr_p]
        M12, M13, M23 = [], [], []
        for (c1, c2) in pairs:
...
def certify_G1(cache, ell_lo, ell_hi, max_boxes=800000, progress=True):
    ...
    while stack:
        ...
        if ok:
            ...
            cl = NCv.scale_int(3) / S2v.scale_int(4)
            cp = NCpv.scale_int(3) / S2v.scale_int(4)
            dlt = DNv.scale_int(3) / (denv.sq() * S2v).scale_int(4)
            slope = dlt / cl
...
def main():
    ...
    t0 = time.time()
    print("Certificate v2: enlarged-region global identification")
    print("B = [1,5] x [5,15] x [0,3/20]  (lambda, eta, psi);  PREC=%d, N=%d" % (PREC, N_TRUNC))
    ...
    if not s1["cl_lo"] > Fraction(1, 50):
        raise RuntimeError("gate C_lambda > 1/50 failed")
    if not s1["cp_lo"] > Fraction(1, 12):
        raise RuntimeError("gate C_psi > 1/12 failed")
    ...
    print("  C_lambda >= %.6f  (gate 1/50) ; C_psi >= %.6f  (gate 1/12)"
    ...
    print("PASS: M3 injective on B (psi=0 included); sigma_min >= 1/100 on B")
